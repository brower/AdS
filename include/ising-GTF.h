...
void QfeIsing::SumLinkGTF(std::vector<int> &sum, int num_dirs) {
  ...
  sum.resize(2*num_dirs + 1) ;
  sum.assign(2*num_dirs + 1, 0) ;
  ...
  for (int s = 0; s < lattice->n_sites; s++) {
    ...
    if (num_links >= 1) {
      ...
      // n.b. There is wasted effort here because positive and negative directions
      // are both summed even though they should be the same. 
      for (int n = 0;  n < num_links; n++) {
        int dir = lattice->sites[s].linkdirs[n] ;
        if (dir > num_dirs || dir < -num_dirs) {
          std::cerr << "SumLink: site " << s << " link " << n << " dir " << dir
